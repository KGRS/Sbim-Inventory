...
import static MainFiles.IndexPage.issueWithoutMaterialRequisitionItemCodeWise;
...
public class IssueWithoutMaterialRequisitionItemCodeWise extends javax.swing.JInternalFrame {
    ...
    private final String internalFrameName = "Issue without Material Requisition (Item code wise)";

    /**
     * Creates new form ItemDisposal
     */
    public IssueWithoutMaterialRequisitionItemCodeWise() {
        initComponents();

        LoadTakeFromToCombo();
        LoadSystemDate();
        LoadTaxesToLables();
        model_DisposalItemTable = (DefaultTableModel) tableIssueItem.getModel();
    ...
    private void initComponents() {
        ...
        tableIssueItem = new javax.swing.JTable();
        ...
        lbl_description.setText("Issue Date");
        ...
        lbl_subAccount.setText(" Search items to issue by");
        ...
        PanelPurchaseOrder.setBorder(javax.swing.BorderFactory.createTitledBorder(new javax.swing.border.LineBorder(new java.awt.Color(153, 153, 153), 1, true), "Available items at department"));
        ...
        tableIssueItem.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        tableIssueItem.getTableHeader().setReorderingAllowed(false);
        tableIssueItem.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tableIssueItemMouseClicked(evt);
            }
        });
        tableIssueItem.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                tableIssueItemKeyPressed(evt);
            }
        });
        jScrollPane3.setViewportView(tableIssueItem);
        if (tableIssueItem.getColumnModel().getColumnCount() > 0) {
            tableIssueItem.getColumnModel().getColumn(1).setPreferredWidth(250);
        }
        ...
        panel1.add(textIssueBy, new org.netbeans.lib.awtextra.AbsoluteConstraints(890, 30, 160, -1));

        txtID.setDisabledTextColor(new java.awt.Color(0, 0, 0));
        txtID.setEnabled(false);
        panel1.add(txtID, new org.netbeans.lib.awtextra.AbsoluteConstraints(569, 610, 110, -1));

        lbl_category8.setForeground(new java.awt.Color(102, 102, 102));
        lbl_category8.setText(" Issue to (Department)");
        ...
        lbl_category9.setText(" Issue to (Branch)");
    ...
    private void LoadTakeFromDepartmentToCombo() {
        try {
            String TakeFromBranchCode[] = cmbTakeFromBranch.getSelectedItem().toString().split("--");
            java.sql.Statement stmt = ConnectSql.conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            String query = "select DepartmentName, DepartmentCode From Departments where BranchCode = '" + TakeFromBranchCode[1] + "' order by DepartmentName";
            ResultSet rset = stmt.executeQuery(query);
    ...
    private void CheckBeforeSave() {
        int TransferRowCount = tableIssueItem.getRowCount();
        String RequestedBy = textIssueBy.getText();
        if (cmbTakeFromDepartment.getSelectedItem().equals(select)) {
            ...
            JOptionPane.showMessageDialog(this, "Name for issue by is not inserted.", "Empty name", JOptionPane.OK_OPTION);
            ...
            if (OtherChargers < 0) {
                ...
                int x = JOptionPane.showConfirmDialog(this, "Are you sure to save this '" + internalFrameName + "'?", "Save Item Disposal?", JOptionPane.YES_NO_OPTION);
    ...
    private void LoadTime() {
        ...
        try {
            ...
            if (resultset.next()) {
                ...
                Time = Time.split("\\.")[0];
                SaveAsIssueWithoutMRN(Time, Date);
            }
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            JOptionPane.showMessageDialog(this, "please contact for support.");
    ...
    private void SaveAsIssueWithoutMRN(String Time, String Date) {
        String ItemCode, ItemName, PurchaseUnitCode, UnitSell, MRNID = "NO MRN";
        double PurchasePrice, Quantity, Amount, SellPrice;
        double CurrentQuantity, NewQuantity;
        double CurrentQuantityAtDepartment, NewQuantityAtDepartment;
        int PriceLevel = 0;
        ResultSet RSETCurrentItemQuantity, RSETItemAlreadyAtDepartment;

        try {
            String TakeFromDepartmentCode[] = cmbTakeFromDepartment.getSelectedItem().toString().split("--");
            ...
            //////////////////////////////////////////////////////////////////
            int RowCount = tableIssueItem.getRowCount();
            ...
            java.sql.Statement stmtItemAlreadyAtDepartment = ConnectSql.conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);

            for (int i = 0; i < RowCount; i++) {
                ItemCode = tableIssueItem.getValueAt(i, 0).toString();
                ItemName = tableIssueItem.getValueAt(i, 1).toString();
                PurchaseUnitCode = tableIssueItem.getValueAt(i, 2).toString();
                PurchasePrice = roundTwoDecimals(Double.parseDouble(tableIssueItem.getValueAt(i, 3).toString()));
                Quantity = roundThreeDecimals(Double.parseDouble(tableIssueItem.getValueAt(i, 4).toString()));
                Amount = roundTwoDecimals(Double.parseDouble(tableIssueItem.getValueAt(i, 5).toString()));
                ...
                String ItemAlreadyAtDepartmentQuery = "SELECT ItemCode, DepartmentCode, Quantity FROM ItemsAtDepartments WHERE ItemCode='" + ItemCode + "' AND DepartmentCode ='" + TakeFromDepartmentCode[1] + "'";
                RSETItemAlreadyAtDepartment = stmtItemAlreadyAtDepartment.executeQuery(ItemAlreadyAtDepartmentQuery);

                if (RSETCurrentItemQuantity.next() && RSETItemAlreadyAtDepartment.next()) {
                    CurrentQuantityAtDepartment = RSETItemAlreadyAtDepartment.getDouble("Quantity");
                    CurrentQuantity = RSETCurrentItemQuantity.getDouble("Quantity");
                    NewQuantity = roundThreeDecimals(CurrentQuantity - Quantity);
                    NewQuantityAtDepartment = roundThreeDecimals(CurrentQuantityAtDepartment + Quantity);
                    ...
                    String ItemsAtDepartmentsUpdate = "UPDATE [ItemsAtDepartments]\n"
                            + "   SET [SellPrice] = '" + SellPrice + "'\n"
                            + "      ,[Quantity] = '" + NewQuantityAtDepartment + "'\n"
                            + "      ,[UnitSell] = '" + UnitSell + "'\n"
                            + " WHERE [ItemCode] = '" + ItemCode + "' AND [DepartmentCode] = '" + TakeFromDepartmentCode[1] + "'";
                    stmtItemsAtDepartmentsQty.execute(ItemsAtDepartmentsUpdate);
                ...
                } else if (!RSETItemAlreadyAtDepartment.next()) { 
                    CurrentQuantity = RSETCurrentItemQuantity.getDouble("Quantity");
                    NewQuantity = roundThreeDecimals(CurrentQuantity - Quantity);
                    SellPrice = RSETCurrentItemQuantity.getDouble("SellPrice");
                    UnitSell = RSETCurrentItemQuantity.getString("UnitSell");
            ...
            stmtItemAlreadyAtDepartment.close();
...
//            LoadItems();
    ...
    private void exit() {
        int GRnRowCount = tableIssueItem.getRowCount();
        ...
        if (GRnRowCount > 0 && GRNID.isEmpty()) {
            ...
            if (x == JOptionPane.YES_OPTION) {
                if (issueWithoutMaterialRequisitionItemCodeWise != null) {
                    issueWithoutMaterialRequisitionItemCodeWise = null;
                }
                this.dispose();
            }
    ...
    private void ButtonPreviewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonPreviewActionPerformed
        String transactionID = txtID.getText();
    ...
    private void SaveForTestPreview(String Time, String Date) {
        ...
        try {
            ...
            for (int i = 0; i < rowCount; i++) {
                ...
                Quantity = roundThreeDecimals(Double.parseDouble(tableIssueItem.getValueAt(i, 4).toString()));
                Amount = roundTwoDecimals(Double.parseDouble(tableIssueItem.getValueAt(i, 5).toString()));
                ExpireDate = tableIssueItem.getValueAt(i, 6).toString();
    ...
    private void TableAvailableItemMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TableAvailableItemMouseClicked
        ...
        tableIssueItem.setSelectionMode(0);
    ...
    private void tableIssueItemMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableIssueItemMouseClicked
        if (evt.getClickCount() == 1) {
            TableAvailableItem.setSelectionMode(0);
            buttonSave.setEnabled(false);
            String PurchasePrice, Quantity, Amount;
            float CheckAmount = Float.parseFloat(tableIssueItem.getValueAt(tableIssueItem.getSelectedRow(), 5).toString());
            if (CheckAmount > 0) {
                PurchasePrice = tableIssueItem.getValueAt(tableIssueItem.getSelectedRow(), 3).toString();
                Quantity = tableIssueItem.getValueAt(tableIssueItem.getSelectedRow(), 4).toString();
                Amount = tableIssueItem.getValueAt(tableIssueItem.getSelectedRow(), 5).toString();
                TextPurchasePrice.setText(PurchasePrice);
                TextQuantity.setText(Quantity);
            ...
            if (x == JOptionPane.YES_OPTION) {
                int i = tableIssueItem.getSelectedRow();
    ...
    }//GEN-LAST:event_tableIssueItemMouseClicked

    private void tableIssueItemKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tableIssueItemKeyPressed
    ...
    }//GEN-LAST:event_tableIssueItemKeyPressed

    private void CalculateAmountWithOutTax() {

        float ItemPriceAmount;
        float TotalWithoutTaxes = 0;
        int GRNTableRowCount = tableIssueItem.getRowCount();
        for (int i = 0; i < GRNTableRowCount; i++) {
            ItemPriceAmount = Float.parseFloat(tableIssueItem.getValueAt(i, 5).toString());
    ...
    private void ButtonAddSelectedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonAddSelectedActionPerformed
        ...
        int SelectedRowCountAtTransaction = tableIssueItem.getSelectedRowCount();
    ...
    private void FirstCheckAtUpdateButtonBeforeUpdateWithTransactionQuantity() {
        buttonSave.setEnabled(false);
        int SelectedRowCount = tableIssueItem.getSelectedRowCount();
        if (SelectedRowCount == 1) {
            if (!TextPurchasePrice.getText().equals("") && !TextQuantity.getText().equals("") && !TextAmount.getText().equals("")) {
    ...
    private void CheckAtUpdateButtonBeforeUpdate() {
        if (!TextPurchasePrice.getText().equals("") && !TextQuantity.getText().equals("") && !TextAmount.getText().equals("")) {
            ...
            int getSelectedRowAtTransactionTable = tableIssueItem.getSelectedRow();
            try {
                ItemCode = tableIssueItem.getValueAt(getSelectedRowAtTransactionTable, 0).toString();
                ...
                if (RSETAvaItems.next()) {
                    ...
                    if (AvailableQuantity > 0 && AvailableQuantity >= Quantity) {
                        ...
                        JOptionPane.showMessageDialog(this, "issue quantity should be less than or equal to the available quantity.", "Not enough.", JOptionPane.OK_OPTION);
                    }
                }
                stmtItems.close();
                RSETAvaItems.close();
    ...
    private void UpdateToReturnTable() {
        int SelectedRow = tableIssueItem.getSelectedRow();
...
//        float Amount = Float.parseFloat(TextAmount.getText().toString());
        ...
        tableIssueItem.setValueAt(PurchasePrice, SelectedRow, 3);
        tableIssueItem.setValueAt(Quantity, SelectedRow, 4);
        tableIssueItem.setValueAt(Amount, SelectedRow, 5);
        CalculateAmountWithOutTax();
    }
    ...
    private void CheckAtQuantityFieldBeforeUpdateWithTransactionQuantity() {
        if (!TextPurchasePrice.getText().equals("") && !TextQuantity.getText().equals("") && !TextAmount.getText().equals("")) {
            ...
            if (TransferQuantity > 0) {
                try {
                    String ItemCode = tableIssueItem.getValueAt(SelectedRow, 0).toString();
                    ResultSet RSETAvaItems;
                    java.sql.Statement stmtGRNItems = ConnectSql.conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                    String ItemQuery = "SELECT Quantity FROM Items WHERE ItemCode='" + ItemCode + "'";
                    RSETAvaItems = stmtGRNItems.executeQuery(ItemQuery);
    ...
    private void formInternalFrameIconified(javax.swing.event.InternalFrameEvent evt) {//GEN-FIRST:event_formInternalFrameIconified
        issueWithoutMaterialRequisitionItemCodeWise.toFront();
...
//        model_DisposalItemTable = (DefaultTableModel) TableDisposalItem.getModel();
    ...
    public static javax.swing.JTable tableIssueItem;
