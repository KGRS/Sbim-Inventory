 ...
 * @author Ravindu
 */
public class CommonItemSearch extends javax.swing.JDialog {

    private GRN object_GRN;
    private final DefaultTableModel model_ItemTable;
    private DefaultTableModel model_POTable;
    private DefaultTableModel model_FreeItemsExisting;
    private DefaultTableModel model_FreeItemsNew;

    private PurchaseOrder purchaseOrderObject = null;
    private AddFreeItems addFreeItemsObject = null;
    private final String projectPath = System.getProperty("user.dir");

    /**
     * Creates new form GRNItemSearch
     *
     * @param objectCommonSearch
     */
    public CommonItemSearch(Object objectCommonSearch) {
        super();

        if (objectCommonSearch instanceof PurchaseOrder) {
            this.purchaseOrderObject = (PurchaseOrder) objectCommonSearch;
        } else if (objectCommonSearch instanceof AddFreeItems) {
            this.addFreeItemsObject = (AddFreeItems) objectCommonSearch;
        }
...
//        this.setTitle("Item search for GRN");
    ...
    private void initComponents() {
        ...
        setTitle("Common item search");
        setAlwaysOnTop(true);
        setModal(true);
        setModalExclusionType(java.awt.Dialog.ModalExclusionType.APPLICATION_EXCLUDE);
        setPreferredSize(new java.awt.Dimension(1070, 449));
        setResizable(false);
        ...
        panel1.add(TextQuantity, new org.netbeans.lib.awtextra.AbsoluteConstraints(700, 20, 110, -1));
        ...
        panel1.add(TextAmount, new org.netbeans.lib.awtextra.AbsoluteConstraints(830, 20, 110, -1));
        ...
        panel1.add(TextPurchasePrice, new org.netbeans.lib.awtextra.AbsoluteConstraints(570, 20, 110, -1));
    ...
    private void btnAddToGRNActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddToGRNActionPerformed
        ...
        if (SelectedRowCount == 1) {
            String addItemToTransaction = TableItem.getValueAt(TableItem.getSelectedRow(), 0).toString();
            if (this.purchaseOrderObject instanceof PurchaseOrder) {
//                model_POTable = (DefaultTableModel) PurchaseOrder.tablePurchaseOrder.getModel();
                Object[] CheckItemAlreadyAdded = CheckItemAlreadyAddedToPurchaseOrder(addItemToTransaction);
            ...
            } else if (this.addFreeItemsObject instanceof AddFreeItems) {
//                model_FreeItemsExisting = (DefaultTableModel) AddFreeItems.tableExistingItems.getModel();
//                model_FreeItemsNew = (DefaultTableModel) AddFreeItems.tableNewFreeItems.getModel();
                Object[] CheckItemAlreadyAddedToExistingFreeItems = CheckItemAlreadyAddedToExistingFreeItems(addItemToTransaction);
                Object[] CheckItemAlreadyAddedToNewFreeItems = CheckItemAlreadyAddedToNewFreeItems(addItemToTransaction);
                if ((Boolean) CheckItemAlreadyAddedToExistingFreeItems[0]) {
                    JOptionPane.showMessageDialog(this, "Item is already added to existing free items.", "Already added.", JOptionPane.OK_OPTION);
                } else if ((Boolean) CheckItemAlreadyAddedToNewFreeItems[0]) {
                    JOptionPane.showMessageDialog(this, "Item is already added to new free items.", "Already added.", JOptionPane.OK_OPTION);
                } else {
                    AddAsNewFreeItems();
                }
            }
    ...
    protected Object[] CheckItemAlreadyAddedToPurchaseOrder(String ItemToPurchaseOrder) {
        model_POTable = (DefaultTableModel) PurchaseOrder.tablePurchaseOrder.getModel();
        int rowCount = model_POTable.getRowCount();
        Object[] data = new Object[2];
        data[0] = false;
        data[1] = -1;

        for (int i = 0; i < rowCount; i++) {
            String GRNTableItemCode = model_POTable.getValueAt(i, 0).toString();
            if (ItemToPurchaseOrder.equals(GRNTableItemCode)) {
                data[0] = true;
                data[1] = i;
            }
        }
    ...
    protected Object[] CheckItemAlreadyAddedToExistingFreeItems(String existingFreeItem) {
        model_FreeItemsExisting = (DefaultTableModel) AddFreeItems.tableExistingItems.getModel();
        int rowCount = model_FreeItemsExisting.getRowCount();
        Object[] data = new Object[2];
        data[0] = false;
        data[1] = -1;

        for (int i = 0; i < rowCount; i++) {
            String otherTableItemCode = model_FreeItemsExisting.getValueAt(i, 0).toString();
            if (existingFreeItem.equals(otherTableItemCode)) {
                data[0] = true;
                data[1] = i;
            }
        }
    ...
    protected Object[] CheckItemAlreadyAddedToNewFreeItems(String newFreeItem) {
        model_FreeItemsNew = (DefaultTableModel) AddFreeItems.tableNewFreeItems.getModel();
        int rowCount = model_FreeItemsNew.getRowCount();
        Object[] data = new Object[2];
        data[0] = false;
        data[1] = -1;

        for (int i = 0; i < rowCount; i++) {
            String otherTableItemCode = model_FreeItemsNew.getValueAt(i, 0).toString();
            if (newFreeItem.equals(otherTableItemCode)) {
                data[0] = true;
                data[1] = i;
            }
        }
    ...
    private void AddToPurchaseOrder() {
        ...
        if (!TextPurchasePrice.getText().equals("") && !TextQuantity.getText().equals("") && !TextAmount.getText().equals("")) {
            ...
            if (CheckQuantity <= 0 || CheckPurchasePrice <= 0) {
                ...
                if (this.purchaseOrderObject instanceof PurchaseOrder) {
                    model_POTable = (DefaultTableModel) PurchaseOrder.tablePurchaseOrder.getModel();
                    model_POTable.addRow(new Object[]{ItemCode, ItemName, PurchaseUnitCode, PurchasePrice, Quantity, Amount, ExpireDate});
                    PurchaseOrder.buttonSave.setEnabled(false);
                    CalculateAmountWithOutTax();
//                    object_GRN.countItemsInPOItemsTable();
                }
            }
    ...
    private void CalculateAmountWithOutTax() {
        ...
        if (this.purchaseOrderObject instanceof PurchaseOrder) {
            int POTableRowCount = PurchaseOrder.tablePurchaseOrder.getRowCount();
            for (int i = 0; i < POTableRowCount; i++) {
                ItemPriceAmount = Float.parseFloat(PurchaseOrder.tablePurchaseOrder.getValueAt(i, 5).toString());
                TotalWithoutTaxes = TotalWithoutTaxes + ItemPriceAmount;
            }
            PurchaseOrder.txtTotalNoTax.setText(String.valueOf(TotalWithoutTaxes));
        }
        Refresh();
        txtSearch.requestFocus();
        txtSearch.selectAll();
    ...
    private void AddAsNewFreeItems() {
        String ItemCode, ItemName, PurchaseUnitCode;
        String PurchasePrice, Quantity;
        ...
        if (!TextPurchasePrice.getText().equals("") && !TextQuantity.getText().equals("") && !TextAmount.getText().equals("")) {
            ...
            if (CheckQuantity <= 0 || CheckPurchasePrice <= 0) {
                ...
                if (this.addFreeItemsObject instanceof AddFreeItems) {
                    model_FreeItemsNew = (DefaultTableModel) AddFreeItems.tableNewFreeItems.getModel();
                    model_FreeItemsNew.addRow(new Object[]{ItemCode, ItemName, PurchaseUnitCode, PurchasePrice, Quantity});
                    Refresh();
                }
            }
